...
class Bitmap {
  constructor(filePath, buffer) {
    ...
    for (let offset32s in bitmap32OffsetsSigned) {
      this.info[offset32s] = buffer.readInt32LE(bitmap32OffsetsSigned[offset32s]);
    }

    for (let offset32u in bitmap32OffsetsUnsigned) {
      this.info[offset32u] = buffer.readUInt32LE(bitmap32OffsetsUnsigned[offset32u]);
    }
    ...
    if ( [3, 6].includes(this.info.compressionMethod) ) {
      for (let bitmask in bitmasks) {
        this.info[bitmask] = buffer.readUInt32LE(bitmasks[bitmask]);
      }
    }
    ...
    if (this.info.paletteColorCount) {
      ...
      this.info.colorPaletteOffset = 14 + this.info.headerSize + 4 * this.file.info.compressionMethod;
   ...
   * Returns the masked number, only of the masked bits
   * e.g. binary representations:
   * 0011100 & 1110100 = 0010100 -> 101 = 5
   * @param {int} bitmask Bitmask to work with
   * @param {int} unmasked Number to mask
   * @param {int} length Total length of unmasked number's binary representation
   * @returns {int}
   */
  bitmaskToNumber(bitmask, unmasked, length) {
    let masked = (bitmask & unmasked).toString(2).padStart(length, '0');
    let substring = masked.substring(masked.indexOf('1'), masked.lastIndexOf('1') + 1);
  ...
  pixelColor(x, y, color = null) {
    ...
    case 8: {
      ...
      let index = this.buffer.readUInt8(offset);
    ...
    case 16: {
      ...
      // pixelColor = this.buffer.readUInt16LE(offset).toString(2);
      pixelColor = this.buffer.readUInt16LE(offset);
      this.bitmaskToNumber(this.info.bitmaskR, pixelColor, this.info.bitDepth);
      
      // let pixel = this.buffer.readUInt16LE(offset)
      // pixelColor = {
      //   r: (pixel & this.info.bitmaskR),
